...
Current milestone: **v0.8.0 — Primary-entity marking.** ✅ shipped 2026-05-27. Closes the alphabetical-sort gap that surfaced during meta-vault dogfooding: when a vault has more than one persona, product, or competitor, the doc-generators used to pick one by id sort — which produced wrong representatives whenever entities were named semantically rather than by priority prefix. v0.8.0 adds an optional `primary: bool` field on the base frontmatter; five generators (one-pager, MRD, sales-battle-card, press-release, onboarding-doc) check it first and fall back to alphabetical with a footer note. `cb validate --strict` warns on over-marking and on rendered exports with no primary marked. Backward compatible — pre-existing vaults render unchanged except for the new fallback note. See the [CHANGELOG](CHANGELOG.md) for the full delta. Next milestone is **v1.0.0** — the release tag and public announcement.
